...
CREATE OR REPLACE PACKAGE BODY TEST_PKG_TEST_ME AS
  ...
  BEGIN
    ...
    UT.EXPECT(CODE_OWNER.PKG_TEST_ME.FC_TEST_ME('1')).TO_EQUAL(1);
  END;
  ...
  BEGIN
    ...
    UT.EXPECT(CODE_OWNER.PKG_TEST_ME.FC_TEST_ME('0')).TO_EQUAL(0);
  END;
  ...
  BEGIN
    ...
    UT.EXPECT(CODE_OWNER.PKG_TEST_ME.FC_TEST_ME(NULL)).TO_BE_NULL;
  ...
  BEGIN
    ...
    SELECT COUNT(1) INTO VNCOUNT1 FROM CODE_OWNER.TO_TEST_ME;
    CODE_OWNER.PKG_TEST_ME.PR_TEST_ME(NULL);
    SELECT COUNT(1) INTO VNCOUNT2 FROM CODE_OWNER.TO_TEST_ME;
  ...
  PROCEDURE TEST_PR_TEST_ME_NOT_NULL IS
    VNCOUNT1 PLS_INTEGER;
    VNCOUNT2 PLS_INTEGER;
    VSNAME   CODE_OWNER.TO_TEST_ME.SNAME%TYPE;
  BEGIN
    ...
    CODE_OWNER.PKG_TEST_ME.PR_TEST_ME(VSNAME);
    SELECT COUNT(1) INTO VNCOUNT2 FROM CODE_OWNER.TO_TEST_ME;
  
    -- Check that I have inserted the value
    UT.EXPECT(VNCOUNT1 + 1).TO_EQUAL(VNCOUNT2);
    SELECT COUNT(1) INTO VNCOUNT2 FROM CODE_OWNER.TO_TEST_ME T WHERE T.SNAME = VSNAME;
  
    -- Check that I inserted the one I said I would insert
    UT.EXPECT(VNCOUNT2).TO_EQUAL(1);
    DELETE FROM CODE_OWNER.TO_TEST_ME T WHERE T.SNAME = VSNAME;
    COMMIT;
  END;
  ...
  BEGIN
    ...
    BEGIN
      CODE_OWNER.PKG_TEST_ME.PR_TEST_ME('EXISTS');
      CODE_OWNER.PKG_TEST_ME.PR_TEST_ME('EXISTS');
    EXCEPTION
      WHEN OTHERS THEN
  ...
  BEGIN
    ...
    OPEN VEXPECTED FOR
      SELECT T.SNAME FROM CODE_OWNER.TO_TEST_ME T;
    OPEN VACTUAL FOR
      SELECT T.SNAME FROM CODE_OWNER.TO_TEST_ME T;
